...
func SplitIntoFiles(path string) error {
	data, count, err := Split(path)
	if err != nil {
		return err
	}
	err = StorePartitions(data, count, path)
	if err != nil {
		return err
	}
	return nil
...
func Split(path string) ([]*[]byte, int, error) {
...
func StorePartitions(chunks []*[]byte, count int, path string) error {
